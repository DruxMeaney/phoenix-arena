...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("phoenix_admin_pass") === "1234") {
  ...
  return (
    <div className="fixed inset-0 z-40 flex overflow-hidden bg-[#070711] text-foreground">
      ...
      <aside className="hidden lg:flex flex-col w-72 border-r border-border bg-surface/70 backdrop-blur-xl fixed inset-y-0 left-0 z-30">
        ...
        <div className="p-5 border-b border-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center shrink-0">
        ...
        <div className="p-3 border-t border-border">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-muted hover:text-red-400 hover:bg-red-600/10 transition-all"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-surface/95 backdrop-blur-xl border-b border-border">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-foreground">Phoenix Admin</span>
          </div>
          <button
        ...
        {mobileMenuOpen && (
          <div className="border-t border-border px-3 py-2 space-y-1 max-h-[60vh] overflow-y-auto">
            {SECTIONS.map((s) => (
              <button
                key={s.key}
                onClick={() => { setSection(s.key); setMobileMenuOpen(false); }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${
                  section === s.key
                    ? "bg-red-600/15 text-red-400 font-medium"
                    : "text-muted hover:text-foreground hover:bg-surface-2/50"
            ...
            <button
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-muted hover:text-red-400"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
              </svg>
      ...
      <main className="h-dvh min-h-dvh flex-1 overflow-y-auto pt-14 lg:ml-72 lg:pt-0">
        <div className="p-4 sm:p-6 lg:p-8 xl:pr-[22rem] 2xl:pr-[23rem]">
          ...
          {section === "transacciones" && <AdminTransactions />}
          {section === "actividad" && <AdminActivity />}
        </div>
      </main>
    </div>
  );
