...
const errorMessages: Record<string, string> = {
  discord_denied: "Cancelaste la autorizacion de Discord.",
  no_code: "No se recibio el codigo de autorizacion.",
  invalid_state: "Token de seguridad invalido. Intenta de nuevo.",
  token_exchange_failed: "Error al conectar con Discord. Intenta de nuevo.",
  user_fetch_failed: "No se pudo obtener tu perfil de Discord.",
};
...
function LoginContent() {
  ...
  const showDevLogin = process.env.NEXT_PUBLIC_ENABLE_DEV_LOGIN === "1";
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-muted mt-2">Inicia sesion en tu cuenta de Phoenix Arena</p>
        </div>

        {/* Error banner */}
        {errorParam && errorMessages[errorParam] && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-start gap-3">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-red-400 shrink-0 mt-0.5">
              <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
            </svg>
            <p className="text-sm text-red-300">{errorMessages[errorParam]}</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs text-muted uppercase tracking-wider">o con tu correo</span>
          <div className="flex-1 h-px bg-border" />
        </div>
        ...
        >
          ...
          <button
            type="submit"
            className="w-full py-3.5 rounded-xl bg-gradient-main text-white font-semibold hover:opacity-90 transition-opacity shadow-lg"
          >
            Iniciar sesion
          </button>
        </form>
