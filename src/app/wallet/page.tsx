...
export default function WalletPage() {
  ...
  const [withdrawalCommissionRate, setWithdrawalCommissionRate] = useState(0.05);
  ...
  const fetchWallet = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (typeof data.withdrawalCommissionRate === "number") {
          setWithdrawalCommissionRate(data.withdrawalCommissionRate);
        }
        ...
        {activeTab === "depositar" ? (
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 sm:p-8 space-y-6">
            ...
            {withdrawAmount && parseFloat(withdrawAmount) > 0 && (() => {
              ...
              const commission = Math.round(requested * withdrawalCommissionRate * 100) / 100;
              const net = Math.round((requested - commission) * 100) / 100;
              return (
                <div className="bg-surface-2 border border-border rounded-xl p-4 text-sm space-y-1">
                  <div className="flex justify-between text-muted"><span>Solicitado</span><span className="font-mono">${requested.toFixed(2)}</span></div>
                  <div className="flex justify-between text-muted"><span>Comision ({(withdrawalCommissionRate * 100).toFixed(1)}%)</span><span className="font-mono text-red-400">-${commission.toFixed(2)}</span></div>
                  <div className="flex justify-between text-foreground font-semibold border-t border-border pt-1 mt-1"><span>Recibiras</span><span className="font-mono text-success">${net.toFixed(2)}</span></div>
                </div>
              );
            })()}
